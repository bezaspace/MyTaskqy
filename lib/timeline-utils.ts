...
import { getCurrentISTDate, formatIST, toIST, IST_TIMEZONE } from './ist-time-utils';
...
export function generateTimeSlots(): TimeSlot[] {
  ...
  for (let hour = 0; hour < 24; hour++) {
    for (let minute = 0; minute < 60; minute += 30) {
      ...
      // Always display time in IST
      const displayTime = formatIST(new Date(2000, 0, 1, hour, minute), 'h:mm a');
...
export function getPixelPosition(date: Date): number {
  // Convert to IST for pixel calculation
  const istDate = toIST(date);
  const hour = istDate.getHours();
  const minute = istDate.getMinutes();
...
export function calculateTaskPositions(tasks: Task[], selectedDate: Date): TaskPosition[] {
  ...
  // Filter tasks for the selected date (in IST)
  const dayTasks = tasks.filter(task => {
    if (task.status === 'scheduled' && task.scheduledStartTime) {
      return isSameDay(toIST(task.scheduledStartTime), toIST(selectedDate));
    }
    return isSameDay(toIST(task.startTime), toIST(selectedDate));
  });
  ...
  dayTasks.forEach(task => {
    ...
    if (task.status === 'scheduled' && task.scheduledStartTime) {
      // Always use scheduledStartTime for scheduled tasks
      startTime = toIST(task.scheduledStartTime);
      if (task.scheduledEndTime) {
        endTime = toIST(task.scheduledEndTime);
      } else {
        // Default duration for scheduled tasks: 30 minutes
        endTime = new Date(startTime.getTime() + 30 * 60 * 1000);
      }
    } else {
      startTime = toIST(task.startTime);
      if (task.status === 'completed' && task.completedTime) {
        endTime = toIST(task.completedTime);
      } else if (task.status === 'in-progress') {
        // For in-progress tasks, calculate real-time end position
        if (isSameDay(startTime, now)) {
          // Use current IST time for tasks in progress today
          endTime = now;
        } else {
...
export function getNavigationDates() {
  const today = toIST(getCurrentISTDate());
